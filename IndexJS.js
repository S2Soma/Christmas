...
function randomWish(name) {
  ...
  if (rate > 82) wishElement.innerHTML = name + wishList[0] + iconList[rate % 7];
  else if (rate > 64) wishElement.innerHTML = wishList[1] + iconList[rate % 7];
  else if (rate > 46) wishElement.innerHTML = "Tặng " + name + wishList[2];
  else if (rate > 28) wishElement.innerHTML = wishList[3] + iconList[rate % 7];
  else if (rate > 18) wishElement.innerHTML = wishList[4] + iconList[rate % 7];
  else wishElement.innerHTML = "Tặng " + name + wishList[5];
}
...
function compareName(name) {
  ...
  if (newName.includes("mến") || newName.includes("men")) {
    wishElement.innerHTML = "Bán mấy cái bằng đi <br/> (-_-)";
    isEqual = true;
  }
    ...
    wishElement.innerHTML = "Tối nay mất con ghẹ :3";
    isEqual = true;
  }
  else if (newName.includes("tiên") || newName.includes("tien")) {
    wishElement.innerHTML = "chị bé Tiên xinh đẹp <br/>tỵt dời (♥_♥)";
    isEqual = true;
  }
  return isEqual;
}
